// Package provider resolves a PassthroughModel provider spec into the
// validated input for the shared gateway builders. Resolution lives outside
// api/v1alpha1 so the API package stays free of cloud SDKs: the operator is
// the only consumer that resolves; display clients such as the key-manager
// read the effective hostname from PassthroughModelStatus instead.
package provider
...
// Resolved is the validated input to the shared gateway builders.
...
func Resolve(p llmv1alpha1.ProviderSpec) (*Resolved, error) {
	backend := llmv1alpha1.BackendOpenAI
	if p.Backend != nil {
	...
	var err error
	switch backend {
	case llmv1alpha1.BackendOpenAI:
		resolved, err = resolveOpenAI(p)
	case llmv1alpha1.BackendBedrock:
		resolved, err = resolveBedrock(p)
	default:
...
func requireCredential(p llmv1alpha1.ProviderSpec, credentialType string) error {
	...
	if credentialType == llmv1alpha1.CredentialAPIKey && strings.TrimSpace(p.CredentialSecretName) == "" {
		return fmt.Errorf("spec.provider.credentialSecretName must not be empty")
	}
	if credentialType == llmv1alpha1.CredentialWorkloadIdentity && p.CredentialSecretName != "" {
