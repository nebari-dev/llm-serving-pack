...
`spec.provider.backend.bedrock.region`, so an AWS-owned override (an
`amazonaws` name, including VPC endpoint DNS) must contain that region and a
cross-region override is rejected at admission.
...
and [Envoy AI Gateway's Connect AWS Bedrock guide](https://theagentrouter.ai/docs/getting-started/connect-providers/aws-bedrock/).
