...
- **Status.** Phase is `Ready`/`Error` with conditions `UpstreamCredentialResolved`, `BackendConfigured`, `ExternalEndpointReady`, and `InternalEndpointReady`. `UpstreamCredentialResolved=False` reports `SecretNotFound` when the referenced provider Secret is absent and `APIKeyMissing` when its `apiKey` entry is absent or empty; `Unknown` with `LookupFailed` reports a temporary read error. "Upstream" identifies the platform-owned key injected toward the provider, not the inbound client API-key Secrets. This condition is informational and does not block `Ready`. Missing AI Gateway CRDs surface as `ApplyFailed` conditions with a one-minute requeue rather than failing reconciliation outright. This surface-and-requeue handling is the intended operator-wide convention for a degraded gateway-apply. The older LLMModel reconciler instead logs-and-continues for the same case; converging it onto this convention is tracked as a follow-up.
- **Generated gateway resources are reconciled on-change-only.** Because the AI Gateway kinds (`Backend`, `BackendTLSPolicy`, `AIServiceBackend`, `BackendSecurityPolicy`, `AIGatewayRoute`, `SecurityPolicy`) are applied as unstructured objects, `SetupWithManager` registers `For(&PassthroughModel{})`, a watch on operator-managed api-keys Secrets, and a targeted watch that maps provider credential Secret events to the PassthroughModels that reference them. It does not `Owns(...)` the generated gateway resources. If a generated resource is deleted out-of-band, it is not recreated until the next PassthroughModel edit, api-keys Secret change in the namespace, or referenced provider credential Secret change.
