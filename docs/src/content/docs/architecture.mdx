...
title: Architecture
---
import Code from '../../components/ExampleCode.astro';
...
Served models report `SecurityPoliciesReady` in their status conditions. Failed
policy writes set `ApplyFailed` and trigger controller retries. A missing
`SecurityPolicy` CRD sets `GatewayCRDUnavailable`; model provisioning continues
while policy creation is retried every 15 seconds. `Applied` means the policies
were written, not that Envoy has finished applying them to traffic.
...
A PassthroughModel routes the shared `llm.<baseDomain>` / `llm-internal.<baseDomain>` endpoints to an external provider, such as OpenRouter or AWS Bedrock. The operator creates routing and the same two authentication layers used by served models. The key manager lists these providers alongside LLMModels so users can mint keys through the same UI.

<Code code={passthroughExample} lang="yaml" title="examples/passthrough-openrouter.yaml" />

Existing resources keep the OpenAI-compatible API-key defaults. Bedrock selects
Converse and AWS workload identity; [the Bedrock guide](/bedrock/) covers live
model discovery and verification:

<Code code={bedrockExample} lang="yaml" title="examples/passthrough-bedrock.yaml" />
...
| AIServiceBackend (`<name>`) | Selected provider schema, such as OpenAI or AWSBedrock |
| BackendSecurityPolicy (`<name>-upstream-auth`) | Selected provider credential, such as API key or AWS workload identity |
| AIGatewayRoute (`<name>-external` / `-internal`) | Declared-model rule (+`modelsOwnedBy`, on both routes) and/or Host-only catch-all rule |
...
- **Served models win.** Since the Host-matcher removal ([#116](https://github.com/nebari-dev/llm-serving-pack/issues/116)), dispatch is decided by the AI Gateway ext_proc's model registry, not by Gateway API header-count precedence. A request whose `model` is a served or declared id is routed to that model's own rule via `x-ai-eg-model`, regardless of route age.
- **`catchAll: true` renders a Host-only rule; whether unknown models reach it depends on the upstream ext-proc's model registry.** If the ext_proc rejects model ids not registered by any route rule (404, "model not configured in the Gateway") before HTTPRoute matching runs, the catch-all rule never receives traffic. Declared models are unaffected either way. Declare every provider model id explicitly rather than relying on catch-all fall-through.
- **`modelsOwnedBy` is set on both routes.** Each endpoint's `/v1/models` aggregates only the routes declaring its own hostname, so each endpoint carries its own catalog without depending on the other endpoint being enabled.
...
- **Status.** Phase is `Ready`/`Error` with conditions `BackendConfigured`, `ExternalEndpointReady`, `InternalEndpointReady`; missing AI Gateway CRDs surface as `GatewayCRDUnavailable` conditions with a 15-second requeue rather than failing reconciliation outright, and real apply failures as `ApplyFailed` with a one-minute requeue. Both controllers share the reason string and the interval, but the LLMModel reconciler applies this convention to its `SecurityPolicy` only: a failed `AIGatewayRoute` apply is logged and skipped with no condition and no requeue, so a served model can report `Ready` while unroutable. Converging the remaining LLMModel applies is tracked in [#109](https://github.com/nebari-dev/llm-serving-pack/issues/109).
