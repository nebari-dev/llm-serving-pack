...
| `spec.provider.credential.type` | string | No | `APIKey` for OpenAI or `WorkloadIdentity` for Bedrock. Defaults to the supported credential for the selected backend. Unsupported combinations are rejected. |
