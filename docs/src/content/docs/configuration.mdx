...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `spec.provider.backend` | object | No | Selects a typed provider backend. Omitted means the existing OpenAI-compatible behavior. |
| `spec.provider.backend.bedrock.region` | string | Bedrock only | AWS region used for the Bedrock runtime endpoint and SigV4. |
| `spec.provider.credential` | object | No | Selects provider authentication independently. Bedrock uses `WorkloadIdentity`; OpenAI-compatible providers use `APIKey`. |
| `spec.provider.hostname` | string | OpenAI only | Provider hostname, e.g. `openrouter.ai` (bare host, TLS assumed). |
